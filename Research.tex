...
We first define four global requirements who capture the entire project in this section.
We first start with a section that has the requirements for the basic system.
After that we will continue with a section that mention the different requirements for the peer matching algorithm.
We choose to split these requirements up, because of the literature study required to further specify the requirements.
The following two sections will split up these requirements in requirements that are measurable and that are assigned to the appropriate MoSCoW\cite{highsmith2001agile} method category.

\subsection{Global Requirements}
Listed below are the global requirements are partly defined by the orginal assignment and partly defined by discussing with our client.
\TODO{The global requirements need more introduction and there are no actual requirements listed below}
\begin{itemize}
\item User information
\item Usage of the system
\item Kind of system
\item Peer matching
\end{itemize}
...
\TODO{Add different algorithms that are compared}
