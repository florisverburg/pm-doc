...
As can be read in these papers \citep{Breese1998},\citep{Peter2007}, there are roughly three kinds of algorithmic recommendation approaches:
\begin{itemize}
\item Collaborative filtering, is the analysing of current user preferences to predict topics and products a new user might like.
...
We will name the aspects that make them inappropriate and what aspects could be used to formulate our own algorithm.

\subsubsection{Collaborative filtering}
The book Adaptive Web \citep{Peter2007} describes what collaborative filtering techniques are. These techniques generally involve matching the ratings of a current user for objects with those of similar users. In this way it produces recommendations for objects that the users has not yet rated or have not been seen by the active user. Traditionally, the primary technique used to implement a collaborative filtering is the use of k-Nearest-Neighbor (kNN) algorithm. This algorithm compares a target user's profile with the historical profiles of other users in order to find the top k users who have similar tastes or interests.
...
\TODO{Content-based filtering definitie toevoegen}
\TODO{Voorbeeld content-based filtering toevoegen}
\TODO{Bruikbare en niet bruikbare aspecten benoemen en verdedigen}


\subsubsection{Hybrid filtering}
\TODO{Hybrid filtering definitie toevoegen}
