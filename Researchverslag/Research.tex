...
One approach is the use of techniques that are used in dating sites.
Another approach is looking at categorising people into classes which the recommendation algorithm can choose from.

\subsubsection{Dating sites}

\subsubsection{Recommendation algorithms}
Their are three different kinds of approaches for recommendation algorithms:
\begin{itemize}
\item Collaborative filtering\cite{Breese1998}. These methods try to predict the utility of items to a particular user based on a database of user votes from a sample or population of other users.
\item Content-based filtering\cite{Peter2007}. These methods use information provided by the user to determine the utility of items.
\item Hybrid Recommender Systems combine the previous two approaches to predict more accurately.
\end{itemize}
